...
export default function ResultSection(props: ResultSectionProps) {
  ...
  const { className, calculatedResult, latestMilvusTag, config } = props;

  const { ModeEnum } = config.types;
  const { extraNodeKey } = config;
  ...
  // dependency data - v250 needs mode parameter
  const dependencyOverviewParams: any = { dependency, dependencyConfig };
  if (!config.supportsWoodpecker) {
    dependencyOverviewParams.mode = mode;
  }
...
sudo docker compose up -d`,
  ...
  const handleDownloadConfigFile = (type: InstallTypeEnum) => {
    ...
    const generatedConfig = configGenerator(
      configParams,
      dependency,
      mode
    );
  ...
  const localFormatOutOfCalData = (params: { data: string; isOut: boolean }) => {
    return formatOutOfCalData ? formatOutOfCalData(params) : (params.isOut ? '--' : params.data);
  };
